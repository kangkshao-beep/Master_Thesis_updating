...
\label{chapter: D_meson_case_fitting}
...
\begin{equation}
    ...
    \begin{equation}
        ...
        此外，归一化能矩本质上是一个比值：
而在 HQE 中，\(M_n\) 与 \(\Gamma_{\rm cut}\) 本身都已经按 \(\alpha_s\) 与 \(1/m_b\) 组织为截断展开，因此在形成比值之后，必须再按同一power counting的规则对其重新展开并截断；否则，若直接将截断后的分子与分母相除，便会自动混入部分更高阶项，从而破坏固定阶计算的一致性。 
归一化能矩不仅反映了衰变宽度与原矩之间微扰行为的收敛性差异，同时也反映了电子能谱中的部分区域关于高阶微扰修正的影响。对于这部分的讨论，需要case by case的进行分析。

这里再给一个具体的例子进行说明re-expansion的必要性: $A=1+a_1\epsilon$,$b=1+b_1\epsilon$ 若讨论他们的比值，R=A/B，直接把固定阶结果拿来做商，则会发生
\begin{equation}
R_{\mathrm{direct}}=\frac{1+a_1 \epsilon}{1+b_1 \epsilon} .
\end{equation}
表面看，分子分母都保留到$\mathcal{O}(\epsilon)$是固定阶，但实际上分母部分的到赎回自动带出无穷多项：
\begin{equation}
\frac{1}{1+b_1 \epsilon}=1-b_1 \epsilon+b_1^2 \epsilon^2-b_1^3 \epsilon^3+\cdots
\end{equation}
此时$R_{\text {direct }}=\left(1+a_1 \epsilon\right)\left(1-b_1 \epsilon+b_1^2 \epsilon^2+\cdots\right)$. 整理有$$
R_{\text {direct }}=1+\left(a_1-b_1\right) \epsilon+\left(b_1^2-a_1 b_1\right) \epsilon^2+\cdots .
$$，尽管输入的分子分母各自精确值线性项，但直接做商显式的引入了$\epsilon^2,\epsilon^3,\cdots$,的项，破坏了固定阶计算的一致性。二阶信息我们不知道，但直接做商之后，却得到了具体的二阶项，该项不是被微扰展开严格算出来的，只是把一阶截断结果做了代数运算引入的。这没有固定阶讨论的意义。我们只讨论截断至线性项的修正，则需要re-expansion，得到
\begin{equation}
R_{\mathrm{re}-\exp }=1+\left(a_1-b_1\right) \epsilon+O\left(\epsilon^2\right) .
\end{equation}
其作用在于将这个ratio重新组织到我们想要讨论的这一阶。这部分对于R值的讨论同样成立。
\subsection{唯象学分析}
...
存在两种方式可得粲介子半轻单举衰变典型能标处的强相互作用耦合常数的解析形式\footnote{分析物理观测量对重整化的依赖，那么可取其显含对数能标依赖的固定阶表达式，然后代入不同能标值已经相应能标处的$\alpha_s$数值（比如 Rundec程序可以输出不同能标处的$\alpha_s$值）；高圈计算过程中，一般不再对$\alpha_s$的能标依赖做截断。当然，我们的做法与高圈计算过程中对$\alpha_s$的能标依赖的处理方式在结果上是自洽的。具体而言，读一读相关的文章去理解RunDec的运行逻辑也是必要的。}: 
\begin{itemize}
