...
QCD中重夸克部分的拉格朗日量为：
\begin{equation}
    \mathcal{L}_{\mathrm{Q}} = \bar{Q}\left(i \slashed{D} - m_{\mathrm{Q}}\right) Q, \quad D_\mu = \partial_\mu + i g_\mrm{s} A_\mu^a T^a,
...
将上式代入QCD拉格朗日量~\eqref{eq:QCD_heavyquark_l}中，并经过必要的化简，得到重夸克有效理论的拉格朗日量
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \operatorname{T}\left\{\mathcal{H}_W^{\dagger}(x), \mathcal{H}_W(0)\right\}=\sum C^n(x) \mathcal{O}_n(0).
    \label{eq:OPE}
\end{equation}
